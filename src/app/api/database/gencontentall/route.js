...
export async function POST(request) {
    try {      
        ...
        for (const url of urls) {
            ...
            const sysPrompt = "ถอดคำและสรุปข่าวนี้ใหม่เป็นภาษาไทย และสร้างหัวข้อของข่าวนี้ ให้เหมือนมนุษย์เขียน 100% เพื่อหลีกเลี่ยงการตรวจสอบจากเครื่องมือตรวจสอบการเขียนด้วย AI ทุกรูปแบบ รูปแบบผลลัพท์ หัวข้อข่าว : , slug (ภาษาอังกฤษ) : , เนื้อหาข่าว : , ข่าวต้นฉบับคือ :"
            ...
            // find 
            const postID = await collectionPosts.findOne({ url: url });
            await collectionPosts.updateOne({ _id: postID }, { $set: { content: 'paraphased' } });
